...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/07/15 17:16:22 by daegulee          #+#    #+#             */
/*   Updated: 2022/07/16 17:08:16 by daegulee         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_upperhex(t_pack *pack)
{
	...
	{
		ui_to_uphp(pack, nb, write_update);
		return ;
	}
	if (pack -> minus)
...
static void	print_minus(t_pack *pack, unsigned int nb, int p_f)
{
	...
	{
		ui_to_uphp(pack, nb, update_w_down);
		w_f = pack -> width;
		while (w_f-- > 0)
			write_update(pack, " ");
	}
